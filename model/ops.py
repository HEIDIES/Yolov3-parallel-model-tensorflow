    ...
    if initializer == 'glorot_normal_tanh':
        ...
        var = tf.get_variable(
            name, shape, initializer=tf.random_normal_initializer(
                mean=mean, stddev=tf.sqrt(2. / (fin + fout)), dtype=DEFAULT_DTYPE
            )
        )
        ...
        var = tf.get_variable(
            name, shape, initializer=tf.random_uniform_initializer(
                minval=-tf.sqrt(6. / (fin + fout)), dtype=DEFAULT_DTYPE
            )
        )
        ...
        var = tf.get_variable(
            name, shape, initializer=tf.random_normal_initializer(
                mean=mean, stddev=4 * tf.sqrt(2. / (fin + fout)), dtype=DEFAULT_DTYPE
            )
        )
        ...
        var = tf.get_variable(
            name, shape, initializer=tf.random_uniform_initializer(
                minval=-4 * tf.sqrt(6. / (fin + fout)), dtype=DEFAULT_DTYPE
            )
        )
        ...
        var = tf.get_variable(
            name, shape, initializer=tf.random_normal_initializer(
                mean=mean, stddev=tf.sqrt(2. / fin), dtype=DEFAULT_DTYPE
            )
        )
        ...
        var = tf.get_variable(
            name, shape, initializer=tf.random_normal_initializer(
                mean=mean, stddev=tf.sqrt(6. / fin), dtype=DEFAULT_DTYPE
            )
        )
    else:
        var = tf.get_variable(
...
def conv2d(x, filters, ksize, stride=1, fixed_pad=True, pad_mode='CONSTANT',
    ...
    :param fixed_pad: bool, size of padding
    ...
    :param data_format: str, channel first or channel last
    :return: 4D tensor, output of convolution layer
    """
    if not data_format or data_format == 'channel_last':
        data_format = 'NHWC'
    else:
        data_format = 'NCHW'
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        if fixed_pad:
            ...
            if data_format == 'NHWC':
                x = tf.pad(x, [[0, 0], [pad_beg, pad_end], [pad_beg, pad_end], [0, 0]], mode=pad_mode)
            else:
...
def unconv2d(x, output_dims, ksize, stride=1, norm=None, activation=None,
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        x = tf.nn.conv2d_transpose(
            ...
            strides=[1, stride, stride, 1], padding='SAME', data_format=data_format
        )
        if use_bias:
            bias = _bias('bias', [output_dims])
            x = tf.add(x, bias)
...
def _norm(x, norm, is_training, activation=None, data_format=None):
    """
...
def _instance_norm(x, activation=None, data_format=None):
    """
    :param x: 2D or 4D tensor
    :param activation: function, the activation function
    :param data_format: str, channel first or channel last
    ...
    with tf.variable_scope('instance_normalization'):
        if not data_format or data_format == 'NHWC':
            axis = [1, 2]
            depth = x.get_shape()[3]
        else:
            axis = [2, 3]
...
def max_pool_with_argmax(x, stride=2, data_format=None):
    """
    :param x: 4D tensor
