...
def c3s1k32(ipt, name='c3s1k32', reuse=False, is_training=True, norm='batch'):
    return ops.conv2d(ipt, 32, 3, 1, True, norm=norm, activation=ops.leaky_relu,
                      name=name, reuse=reuse, is_training=is_training)


def c3s2k64(ipt, name='c3s2k64', reuse=False, is_training=True, norm='batch'):
    return ops.conv2d(ipt, 64, 3, 2, True, norm=norm, activation=ops.leaky_relu,
                      name=name, reuse=reuse, is_training=is_training)


def dark_net_conv_1(ipt, i, name='dark_net_conv_1', reuse=False, is_training=True, norm='batch'):
    c1s1k32 = ops.conv2d(ipt, 32, 1, 1, False, norm=norm, activation=ops.leaky_relu,
                         name=name + str(i), reuse=reuse, is_training=is_training)
    c3s1k64 = ops.conv2d(c1s1k32, 64, 3, 1, True, norm=norm, activation=ops.leaky_relu,
...
def c3s2k128(ipt, name='c3s2k128', reuse=False, is_training=True, norm='batch'):
    return ops.conv2d(ipt, 128, 3, 2, True, norm=norm, activation=ops.leaky_relu,
                      name=name, reuse=reuse, is_training=is_training)


def dark_net_conv_2(ipt, i, j, name='dark_net_conv_2', reuse=False, is_training=True, norm='batch'):
    c1s1k64 = ops.conv2d(ipt, 64, 1, 1, False, norm=norm, activation=ops.leaky_relu,
                         name=name + str(i), reuse=reuse, is_training=is_training)
    c3s1k128 = ops.conv2d(c1s1k64, 128, 3, 1, True, norm=norm, activation=ops.leaky_relu,
                          name=name + str(i+1), reuse=reuse, is_training=is_training)
    if j == 0:
...
def c3s2k256(ipt, name='c3s2k256', reuse=False, is_training=True, norm='batch'):
    return ops.conv2d(ipt, 256, 3, 2, True, norm=norm, activation=ops.leaky_relu,
                      name=name, reuse=reuse, is_training=is_training)


def dark_net_conv_3(ipt, i, j, name='dark_net_conv_3', reuse=False, is_training=True, norm='batch'):
    c1s1k128 = ops.conv2d(ipt, 128, 1, 1, False, norm=norm, activation=ops.leaky_relu,
                          name=name + str(i), reuse=reuse, is_training=is_training)
    c3s1k256 = ops.conv2d(c1s1k128, 256, 3, 1, True, norm=norm, activation=ops.leaky_relu,
                          name=name + str(i+1), reuse=reuse, is_training=is_training)
    return tf.add(c3s1k256, ipt, name='shotcut_' + str(j+3))


def c3s2k512(ipt, name='c3s2k512', reuse=False, is_training=True, norm='batch'):
    return ops.conv2d(ipt, 512, 3, 2, True, norm=norm, activation=ops.leaky_relu,
                      name=name, reuse=reuse, is_training=is_training)


def dark_net_conv_4(ipt, i, j, name='dark_net_conv_4', reuse=False, is_training=True, norm='batch'):
    c1s1k256 = ops.conv2d(ipt, 256, 1, 1, False, norm=norm, activation=ops.leaky_relu,
                          name=name + str(i), reuse=reuse, is_training=is_training)
    c3s1k512 = ops.conv2d(c1s1k256, 512, 3, 1, True, norm=norm, activation=ops.leaky_relu,
                          name=name + str(i+1), reuse=reuse, is_training=is_training)
...
def c3s2k1024(ipt, name='c3s2k1024', reuse=False, is_training=True, norm='batch'):
    return ops.conv2d(ipt, 1024, 3, 2, True, norm=norm, activation=ops.leaky_relu,
                      name=name, reuse=reuse, is_training=is_training)


def dark_net_conv_5(ipt, i, j, name='dark_net_conv_5', reuse=False, is_training=True, norm='batch'):
    c1s1k512 = ops.conv2d(ipt, 512, 1, 1, False, norm=norm, activation=ops.leaky_relu,
                          name=name + str(i), reuse=reuse, is_training=is_training)
    c3s1k1024 = ops.conv2d(c1s1k512, 1024, 3, 1, True, norm=norm, activation=ops.leaky_relu,
                           name=name + str(i+1), reuse=reuse, is_training=is_training)
...
def yolo_1(ipt, num_classes, reuse=False, is_training=True, norm='bath',
    ...
    c1s1k512 = ops.conv2d(ipt, 512, 1, 1, False, norm=norm, activation=activation,
                          name='conv_52', reuse=reuse, is_training=is_training)
    c3s1k1024 = ops.conv2d(c1s1k512, 1024, 3, 1, True, norm=norm, activation=activation,
                           name='conv_53', reuse=reuse, is_training=is_training)
    c1s1k512 = ops.conv2d(c3s1k1024, 512, 1, 1, False, norm=norm, activation=activation,
    ...
    large_obj_raw_detections = ops.conv2d(c3s1k1024, 3 * (num_classes + 5), 1, 1, False, norm=None, activation=None,
                                          use_bias=True,
...
def yolo_2(ipt1, ipt2, num_classes, reuse=False, is_training=True, norm='batch',
           activation=ops.leaky_relu):
    c1s1k256 = ops.conv2d(ipt1, 256, 1, 1, False, norm=norm, activation=activation,
    ...
    c1s1k256 = ops.conv2d(route, 256, 1, 1, False, norm=norm, activation=activation,
                          name='conv_60', reuse=reuse, is_training=is_training)
    c3s1k512 = ops.conv2d(c1s1k256, 512, 3, 1, True, norm=norm, activation=activation,
                          name='conv_61', reuse=reuse, is_training=is_training)
    c1s1k256 = ops.conv2d(c3s1k512, 256, 1, 1, False, norm=norm, activation=activation,
    ...
    medium_obj_raw_detections = ops.conv2d(c3s1k512, 3 * (num_classes + 5), 1, 1, False, norm=None, activation=None,
                                           use_bias=True,
...
def yolo_3(ipt1, ipt2, num_classes, reuse=False, is_training=True, norm='batch',
           activation=ops.leaky_relu):
    c1s1k128 = ops.conv2d(ipt1, 128, 1, 1, True, norm=norm, activation=activation,
    ...
    with tf.variable_scope('route_1'):
        route = tf.concat([upsample, ipt2], axis=3)
    c1s1k128 = ops.conv2d(route, 128, 1, 1, False, norm=norm, activation=activation,
                          name='conv_68', reuse=reuse, is_training=is_training)
    c3s1k256 = ops.conv2d(c1s1k128, 256, 3, 1, True, norm=norm, activation=activation,
                          name='conv_69', reuse=reuse, is_training=is_training)
    c1s1k128 = ops.conv2d(c3s1k256, 128, 1, 1, False, norm=norm, activation=activation,
    ...
    small_obj_raw_detections = ops.conv2d(c3s1k256, 3 * (num_classes + 5), 1, 1, False, norm=None, activation=None,
                                          use_bias=True,
